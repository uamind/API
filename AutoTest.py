...
class TestStudentAPI(unittest.TestCase):
    ...
    def test_home(self):
        ...
        self.assertIn('<th>ID</th><th>Name</th><th>Age</th>', response.text) #判断 response.text 中是否包含特定文本
    ...
    def test_get_students(self):
        ...
        self.assertEqual(response.json()[0]['name'], 'Alice') #判断返回的数据中是否包含正确的姓名
        self.assertEqual(response.json()[0]['age'], 18) #判断返回的数据中是否包含正确的年龄
        # 测试 ID 不存在时返回空数据
    ...
    def test_add_student(self):
        print("测试添加学生信息")
        # 测试合法数据
        new_student = {'id': 4, 'name': 'Dan', 'age': 21}
